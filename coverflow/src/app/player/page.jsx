...
export default function Page() {
    ...
    async function UpdateState()
    {
        const responses = Promise.allSettled([
                GetPlaybackState(localStorage["access"])
            ]).then(async (vals)=>{
    ...
    */
    async function updateQueue(previous_state)
    {
    ...
    return (
        ...
        <motion.div className="text-center justify-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{  }}>
            ...
            >
                ...
                queueData != null && queueData.map((item, index) => {
            ...
            <div className="grid max-cols-1 space-y-3 mt-[20px] h-full w-[40%] mx-[50%] -translate-x-1/2">
                ...
                <>
                    ...
                    <span className="mt-[15px] grid grid-rows-1 grid-cols-3 items-center w-full">
                        <img className="dark:invert justify-self-end max-w-[30px] max-h-[30px]" src="/backward-icon.png" onClick={()=>{albumSwiper.slidePrev()}}></img>
                        {paused ? 
                            <img className="justify-self-center invert dark:invert-0 max-w-[40px] max-h-[40px]" src="/pause.png" onClick={()=>{
                                SetIsPaused(!paused)
                                TogglePlayback(localStorage["access"]).then(v => SetIsPaused(v))
                            }}></img>
                            :
                            <img className="justify-self-center invert dark:invert-0 max-w-[40px] max-h-[40px]" src="/play.png" onClick={()=>{
                                SetIsPaused(!paused)
                                TogglePlayback(localStorage["access"]).then(v => SetIsPaused(v))
                                
                            }}></img>
                        }
                        <img className="dark:invert max-w-[30px] max-h-[30px]" src="/forward-icon.png" onClick={()=>{albumSwiper.slideNext(); updateQueue();}
                        }></img>
                    </span>
                </>
                }
            </div>
        {windowWidth >= 700 && <PlaylistMenu data={playlistData}></PlaylistMenu>}
        </motion.div>
        </>
    )
}