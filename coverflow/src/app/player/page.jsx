                    ...
                    <div className="mb-[50px] grid space-y-5 max-cols-1 w-[40%] max-medium700:w-[80%] mx-[50%] -translate-x-1/2">
                        <>
                            ...
                            <div onMouseDown={()=>SetSeeking(true)} className="progressBarContainer grid mt-[-10px] mb-[5px] items-center h-[30px]">
                                ...
                                <div style={{backgroundColor: `${backgroundColours[0] != undefined && backgroundColours[0] != null ? tinycolor.mostReadable(tinycolor(rgbToHex(backgroundColours[1])), ['rgba(0,0,0,1)', 'rgba(256, 256, 256, 1)']).toRgbString() : "rgba(0,0,0,0)"}`, transform: `translate(${progressBarPosition.current == null ? "0" : Math.min(progressBarPosition.current.getBoundingClientRect().width, progressBar.current.getBoundingClientRect().width) - 1}px,105px)`}}className="invert bg-[#ffffff] !absolute top-0 left-[0px] rounded-full w-[10px] h-[10px] bg-[#000000]"></div>
                            </div>
